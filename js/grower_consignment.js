...
function addEntry() {
    counter++;
    $('#consignment_entry1').clone().find("input").val("").end().appendTo('#consignment_main');
    relabelEntries();
}
...
function relabelEntries() {
    ...
    consignment_form.children().each(function () {
        ...
        $(this).children('h2').html('Entry ' + index);

        var label_index = 1;
        $(this).children('.input_container').each(function () {
            switch (label_index) {
                case 1:
                    $(this).children('.label_name').attr('for', 'fruit_variety' + index);
                    $(this).children('.drop_down').attr('name', 'fruit_variety' + index);
                    break;
                case 2:
                    $(this).children('.label_name').attr('for', 'fruit_size' + index);
                    $(this).children('.drop_down').attr('name', 'fruit_size' + index);
                    break;
                case 3:
                    $(this).children('.label_name').attr('for', 'package_type' + index);
                    $(this).children('.drop_down').attr('name', 'package_type' + index);
                    break;
                case 4:
                    $(this).children('.label_name').attr('for', 'quantity' + index);
                    $(this).children('.input_box').attr('name', 'quantity' + index);
                    break;
                case 5:
                    $(this).children('.label_name').attr('for', 'price' + index);
                    $(this).children('.input_box').attr('name', 'price' + index);
                    break;
            }
            label_index++
        });
